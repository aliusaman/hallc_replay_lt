...
// edited by: nathan heinrich
/******************************/
//Put this file in the shms_hodoscope calibration Folder
/******************************/
...
void WriteFitParamErr(int runNUM)
{
  ...
  TString outPar_Name = Form("../../PARAM/SHMS/HODO/Calibration/phodo_TWcalib_Err_%d.param", runNUM); //note could put this where ever you wanted to
  outParam.open(outPar_Name);
...
void timeWalkCalib_output_errors(int run) {
 ...
 // ROOT settings
  ...
  histoFile = new TFile(Form("./Calibration_Plots/timeWalkHistos_%d.root", run), "READ");
