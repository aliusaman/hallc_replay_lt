/*
	...
	requires you to have run the timeWalkCalib_output_errors.C script first!!!
*/
...
void HodoParamCompair ( TString runNums_name, UInt_t numRuns) //input path to run # file, and the number of runs to look at, and the spectormeter SHMS or HMS
{
	...
	for(UInt_t irun = 0; irun < numRuns; irun++)
	{
		...
		//get file
		fileName = Form("../../PARAM/SHMS/HODO/Calibration/phodo_TWcalib_Err_%.0f.param", runs[irun]); // may need to change this path though
	...
	TCanvas *Temp = new TCanvas(Form("c_s%i_p%i", 1, 1), Form("TW_c2_Comp_"+sideNames[0]+"_side_plane_%i", 2), 1600, 1600); // name, title, width, height
	Temp->SetGrid();
	TGraphErrors *tp = new TGraphErrors (numRuns, runs, Param[1][1][0], nullptr, ParamErr[1][1][0]);
	tp->SetTitle("TW_c2_Comp_Positive_side_plane_1y_Bar_2");
	tp->SetMarkerStyle(1);
	...
	for (UInt_t iside = 0; iside < nSides; iside++)
	{
		...
		for (UInt_t iplane = 0; iplane < nPlanes; iplane++)
		{
			...
			for (UInt_t ibar = 0;((ibar < 13) || (ibar < 14 && iplane == 2)) || (ibar < nBarsMax && iplane == 3) ; ibar++)
			{
				...
				CompGra[iside][iplane][ibar]->SetTitle(Form("TW_c2_Comp_"+sideNames[iside]+"_side_"+planeNames[iplane]+"_Bar%i", ibar+1));
